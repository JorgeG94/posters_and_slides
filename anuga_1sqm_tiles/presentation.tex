...
\titlegraphic{%
  \begin{tabular}{@{}c@{}}
    \begin{tcolorbox}[enhanced,colback=white,colframe=white,boxrule=0pt,
      ...
      \includegraphics[height=1.05cm]{assets/cdac_logo.jpg}
    \end{tcolorbox}%
  \end{tabular}%
}
  ...
  \begin{columns}[T,onlytextwidth]
    \column{.52\textwidth}
      \begin{itemize}\setlength\itemsep{2pt}
        ...
        \item A shared cut line carries the \textbf{finer} lattice --- both tiles compute the same points, so they conform; Triangle grades into the coarse side
        \item Delta test: 1024 / 256 / 64\,m$^2$ $\Rightarrow$ 20M triangles instead of 355M uniform; bit-exact tiled vs merged
        \item Ganges-scale: 2\,M\,km$^2$ is $\sim$3\,T triangles at 1\,m$^2$; at 5\,\% fine, 25\,\% at 25\,m$^2$, rest coarse: $\sim$185\,G
      \end{itemize}
      \muted{Time step: set by triangles at outline/cut-line corners, not by the transitions --- lattice points half a spacing from corners gave 1.38$\times$ on $\Delta t$.}
    \column{.44\textwidth}
  ...
  \begin{columns}[T,onlytextwidth]
    \column{.56\textwidth}
      \begin{itemize}\setlength\itemsep{1pt}
        \item ANUGA's $\Delta t = \min(\text{inradius} / \text{speed})$ over wet cells: the smallest wet triangle decides everything
        \item The smallest triangles were not in the resolution transitions --- they were at \textbf{outline / cut-line corners}: intersections millimetres from an outline vertex, lattice points metres from a corner; Triangle's 28$^\circ$ rule fills those with slivers
        \item Rule: no inserted point within $0.5\,s$ of a segment endpoint, no outline vertex within $s$ of a corner's cut line --- symmetric, so tiles conform
        \item Same mesh: $\Delta t$ 0.032 $\to$ 0.045\,s (1.38$\times$); one triangle in 3.25\,M below half the median inradius
      \end{itemize}
    \column{.4\textwidth}
...
\begin{frame}{Portability}{Where each device lands, and what the AMD gap is}
  \begin{columns}[T,onlytextwidth]
    \column{.56\textwidth}
      ...
      \begin{tabular}{@{}lrrrr@{}}
        \toprule
        device & peak & plateau & HBM & per TB/s \\
        \midrule
        V100 16 GB      & 656  & $\sim$650  & 0.9 & $\sim$720 \\
        A100 80 GB      & 1553 & $\sim$1500 & 2.0 & $\sim$750 \\
        H200 143 GB     & 2843 & $\sim$2550 & 4.8 & $\sim$530 \\
        PVC 64 GB       & 1552 & $\sim$1060 & 1.6 & $\sim$660 \\
        MI250X GCD 64 GB& 419  & $\sim$385  & 1.6 & \textbf{$\sim$240} \\
        \bottomrule
      \end{tabular}
      \muted{Mcell-steps/s, ADER2 + scatter, single device; HBM in TB/s; plateau = large-mesh value. Volume drift $\le 10^{-16}$, zero NaNs, identical max-momentum on every device at all 19 common sizes.}
    \column{.4\textwidth}
      \footnotesize
  ...
  \begin{columns}[T,onlytextwidth]
    \column{.56\textwidth}
      \begin{block}{Expensive: the fit}
        \texttt{set\_quantity(filename, alpha)} is a least-squares fit with smoothing --- a sparse solve over the mesh. At 8.7\,G vertices it cannot run; at 1\,m$^2$ it only blurs the 1\,m LiDAR.
      \end{block}
      \begin{block}{Cheap: sampling per tile}
        \begin{itemize}
          \item Elevation: read a tile's raster window (9\,M cells, 36\,MB), bilinear-sample its 7\,M vertices --- 1--2\,s per tile, same job shape as conversion
          \item Friction: nearest cell per centroid
          \item Rain: cell index per centroid stored once; each 3-hourly update is a GPU gather, milliseconds
        \end{itemize}
      \end{block}
    \column{.4\textwidth}
      \begin{block}{Unstructured sources}
        A hot start from a coarse \texttt{.sww}: do not search 17\,G points in the coarse mesh --- rasterise it once, then sample.
      \end{block}
      \begin{block}{Rule}
